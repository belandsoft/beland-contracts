...
contract MemetaverseNFT is ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyMinter() {
        require(_minters[_msgSender()], "MemetaverseNFT: caller is not minter");
        _;
    }

    modifier onlyCreator() {
        require(creator == _msgSender(), "MemetaverseNFT: caller is not creator");
        _;
    }
    ...
    function addItems(ItemParams[] memory _items) external onlyCreatorOrOwner {
        require(!isApproved, "MemetaverseNFT: not editable");
        ...
        onlyCreatorOrOwner
    {
        ...
        for (uint256 i = 0; i < _itemIds.length; i++) {
            require(items.length > _itemIds[i], "MemetaverseNFT: item not found");
            Item storage item = items[_itemIds[i]];
            require(
                item.totalSupply <= _items[i].maxSupply,
                "MemetaverseNFT: max supply must be greater than total supply"
            );
    ...
    function create(address user, uint256 itemIndex) external onlyMinter {
        require(isApproved, "MemetaverseNFT: not approved");
        _create(user, itemIndex);
    }

    function _create(address user, uint256 itemIndex) private {
        require(items.length > itemIndex, "MemetaverseNFT: item not found");
        require(
    ...
    function setApproved(bool _value) external virtual onlyOwner {
        require(isApproved != _value, "MemetaverseNFT: value is the same");
    ...
    function getItem(uint256 _itemId) external view returns (Item memory) {
        require(items[_itemId].maxSupply > 0, "MemetaverseNFT: item not found");
