...
contract ERC721Bid is Ownable, Pausable, ERC721BidStorage, NativeMetaTransaction {
    ...
    */
    ...
    ) Pausable() NativeMetaTransaction("Memetaverse Bid", "1") {
        ...
        returns (bytes4)
    {
        ...
        if (royaltiesCutPerMillion > 0) {
            ...
            if (success) {
                ...
                require(
                    acceptedToken.transferFrom(bidder, royaltiesReceiver, royaltiesShareAmount),
        ...
        // Fees collector share
        {
            ...
            if (totalFeeCollectorShareAmount > 0) {
                require(
                    acceptedToken.transferFrom(bidder, feesCollector, totalFeeCollectorShareAmount),
                    "ERC721Bid#onERC721Received: TRANSFER_FEES_TO_FEES_COLLECTOR_FAILED"
                );
            }
        }

        // Transfer acceptedToken from bidder to seller
        require(
            acceptedToken.transferFrom(bidder, _from, price - royaltiesShareAmount - feesCollectorShareAmount),
    ...
    * to use bidder MTV on his belhalf
    ...
    */
    function _requireBidderBalance(address _bidder, uint256 _amount) internal view {
        require(
            acceptedToken.balanceOf(_bidder) >= _amount,
            "ERC721Bid#_requireBidderBalance: INSUFFICIENT_FUNDS"
        );
        require(
            acceptedToken.allowance(_bidder, address(this)) >= _amount,
